package com.demo.monesespecex.customviews;
 ...
 * <li>detentWidth (dimension attribute for the width of each transparent detent
 * in the bar)
 * </ul>
 * <p>
...
public class ButteryProgressBar extends View {
	...
	public ButteryProgressBar(Context c, AttributeSet attrs) {
		...
		mBarColor = c.getResources().getColor(R.color.colorAccent);
...
//		mShadow.draw(canvas);
	...
	protected void onDetachedFromWindow() {
		...
		/**
		 * onVisibilityChanged() is also called because of change in visibility of com.android.internal.policy.impl.PhoneWindow$DecorView
		 * which starts animation of this progressbar even though it's not visible. Another case is when we start loading something
		 * on new screen & display this progressbar, at that time if user presses back button then on finishing screen
		 * we should stop animation.
		 * Both above cases will result in leaking memory due to entire activity context held by this progressbar unless
		 * we stop animation by calling stop() as follows
		 */
		stop();
	}
